...
export function App() {
  ...
  const draftSnapshot = (updatedAt = new Date().toISOString()): StoredDraft => ({ allocation, long, short, activeLong, activeShort, enabledLong, enabledShort, planningLeverage, mobileSide, page, updatedAt });
  ...
  const restoreDraft = (stored: StoredDraft | null) => { const next = stored ?? emptyDraft(); setAllocation(next.allocation); setLong(next.long); setShort(next.short); setActiveLong(next.activeLong); setActiveShort(next.activeShort); setEnabledLong(next.enabledLong); setEnabledShort(next.enabledShort); setPlanningLeverage(next.planningLeverage); setMobileSide(next.mobileSide); setPage(next.page ?? "constructor"); setLocalSavedAt(stored ? next.updatedAt : null); };
  ...
  }, [account.symbol, allocation, long, short, activeLong, activeShort, enabledLong, enabledShort, planningLeverage, mobileSide, page]);
  useEffect(() => {
