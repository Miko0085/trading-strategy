...
export function GridChartPreview({ account, long, short, generatedPreview }: { account: AccountState; long: GridOrder[]; short: GridOrder[]; generatedPreview: ShadowProposal | null }) {
  ...
  const body = <><div className="grid-chart-meta"><span>{account.symbol} · {sourceLabel}</span><span>{selectedOrders.length} уровней · расстояния Δ от предыдущего уровня</span></div>{controls}<ChartCanvas symbol={account.symbol} side={side} orders={selectedOrders} markPrice={account.markPrice} interval={interval}/><div className="grid-chart-levels">{selectedOrders.map((item, index) => { const previous = index === 0 ? account.markPrice : selectedOrders[index - 1].price; const gap = previous == null ? null : distancePct(previous, item.price); return <span key={item.level}><b>{side === "long" ? "L" : "S"}{item.level}</b> {item.price.toLocaleString("en-US", { maximumFractionDigits: 8 })}<small>{gap == null ? "—" : "Δ " + gap.toFixed(2) + "%"}</small></span>; })}</div><div className="grid-chart-attribution">Charts by TradingView Lightweight Charts™ · market data: Bybit public API</div></>;
